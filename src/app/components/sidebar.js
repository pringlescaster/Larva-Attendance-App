...
function Sidebar({ closeSidebar }) {
  ...
  return (
    <>
     ...
     {/* Mobile Sidebar */}
<aside className="fixed top-0 left-0 z-50 bg-white px-4 py-8 h-screen w-full md:hidden flex flex-col justify-between overflow-y-auto">
  <div className="flex justify-between items-start mb-4">
    <div className="flex gap-x-[8px]">
      <Avatar name={user?.name || 'Anonymous'} />
      <div className="grid gap-y-[2px]">
        <h1 className="text-[#333333] text-base font-semibold text-left">
          {user?.name || 'Anonymous'}
        </h1>
        <h2 className="text-[#666666] text-sm font-medium text-left">
          {user?.course || 'N/A'}
        </h2>
      </div>
    </div>
